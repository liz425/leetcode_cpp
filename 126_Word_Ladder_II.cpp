...
public:
    ...
    vector<vector<string>> findLadders(string beginWord, string endWord, unordered_set<string> &wordList) {
        ...
        dist[beginWord] = 1;
        queue<string> fringe;
        fringe.push(beginWord);
        int wordLen = (int)beginWord.size();
        //use BFS to construct graph
        while(!fringe.empty()){
            string current = fringe.front();
            if(current == endWord)
                break;
            fringe.pop();
            for(int i = 0; i < wordLen; ++i){
                for(char chr = 'a' ; chr <= 'z'; ++chr){
                    string midWord = current;
                    midWord[i] = chr;
                    if(midWord != current && wordList.find(midWord) != wordList.end()){
                        if(dist[midWord] == 0)
                            fringe.push(midWord);
                        if(dist[midWord] == 0 || dist[current] + 1 == dist[midWord]){
                            dist[midWord] = dist[current] + 1;
                            graph[current].push_back(midWord);
                        }
                    }
                }
            }
        }
        //use DFS to construct result
        vector<string> buf;
        DFS(beginWord, endWord, buf);
        return result;
    }
    void DFS(string beginWord, string endWord, vector<string>& buf){
        //cout << beginWord << endl;
...
public:
    ...
    //for graph, we should not use initialize list, otherwise we need to use explicit constructor.
    unordered_map<string, node> graph;
    vector<vector<string>> result = {};
    

    void getNeighbors(string current, unordered_set<string> &wordList){
        string word = current;
        ...
        for(int i = 0; i < len; ++i){
            ...
            for(char change = 'a'; change <= 'z'; ++change){
                ...
                word[i] = change;
                unordered_set<string>::iterator found = wordList.find(word);
                if(found != wordList.end()){
                    graph[current].neighbors.push_back(*found);
                }
            }
            word = current;
        }
    ...
    vector<vector<string>> findLadders(string beginWord, string endWord, unordered_set<string> &wordList) {
        ...
        //BFS, find shortest pathes
        queue<string> fringe;
        getNeighbors(endWord, wordList);
        
        for(string& word : graph[endWord].neighbors){
            fringe.push(word);
            graph[word].fathers.push_back(endWord);
            graph[word].distance = 1;
        }
        
        ////////////////////////////
        over = clock();
        time_consumed = (double)(over - start) / CLOCKS_PER_SEC * 1000;
        cout << "time in ms: " << time_consumed << endl;
        start = clock();
        ...
        while(!fringe.empty() && !found){
            ...
            for(int i = 0; i < size; ++i){
                ...
                getNeighbors(current, wordList);
                
                for(string& edge : graph[current].neighbors){
                    if(graph[edge].distance == 0){
                        graph[edge].distance = graph[current].distance + 1;
                        graph[edge].fathers.push_back(current);
                        fringe.push(edge);
        ...
        //DFS, output all shortest pathes
        result.clear();
        vector<string> path = {};
    ...
    void DFS(string& current, string& target, vector<string> path){
        ...
        for(string& next : graph[current].fathers){
...
class Solution {
public:
    //TLE!!!!!, 1057 ms in local machine.
    //99% of time consumed in construct graph: 'isValid' function execution in two for-loops.
    struct node{
    ...
    bool isValid(string& firstWord, string& secondWord){
        const int len = (int)firstWord.size();
        int cnt = 0;
        for(int i = 0; i < len; ++i){
            cnt += ((firstWord[i] == secondWord[i])? 0 : 1);
            if(cnt > 1){
                return false;
            }
        }
    ...
    vector<vector<string>> findLadders(string beginWord, string endWord, unordered_set<string> &wordList) {
        ...
        int begin = int(find(wordArray.begin(), wordArray.end(), beginWord) - wordArray.begin());
        int end = int(find(wordArray.begin(), wordArray.end(), endWord) - wordArray.begin());
        
        
        int len = (int)wordArray.size();
        graph.clear();
        graph = vector<node>(len);
        
        for(int i = 0; i < len - 1; ++i){
            for(int j = i + 1; j < len; ++j){
                if(isValid(wordArray[i], wordArray[j])){
        ...
        //BFS, find shortest pathes
        queue<int> fringe;
        for(int& wordIndex : graph[end].neighbors){
            fringe.push(wordIndex);
            graph[wordIndex].fathers.push_back(end);
            graph[wordIndex].distance = 1;
        }
        bool found = false;
        while(!fringe.empty() && !found){
            int size = (int)fringe.size();
            for(int i = 0; i < size; ++i){
                ...
                fringe.pop();
                for(int& edge : graph[current].neighbors){
                    if(graph[edge].distance == -1){
                        graph[edge].distance = graph[current].distance + 1;
                        graph[edge].fathers.push_back(current);
                        fringe.push(edge);
                    }else if(graph[edge].distance == graph[current].distance + 1){
        ...
        result.clear();
        vector<string> path = {};
        DFS(begin, endWord, path);
        return result;
    }
    
    void DFS(int index, string& target, vector<string> path){
        path.push_back(wordArray[index]);
        if(wordArray[index] == target){
            result.push_back(path);
            return;
        }
        for(int& k : graph[index].fathers){
            DFS(k, target, path);
        }
    }
};
...
int main(void){
    ...
    clock_t start, end;
    start = clock();
    vector<vector<string>> result = ss.findLadders(beginWord, endWord, wordList);
    end = clock();
